        ...
        for vol_id in data_list:
            ...
            for bscan, seg_mask, ez, elm in zip(
                ...
                for i in range(n_st):
                    ...
                    ez_roi[np.roll(seg_mask_roi[:,start_r + i * stackwidth: start_r + (i + 1) * stackwidth] != 8, -1)] = np.nan
                    ...
                    # shift the mask 3 position to the left
...
class Patient:
    ...
    def add(self, map: Optional[RelEZI_map] = None, visitdate: Optional[date] = None, vid: Optional[str] = None):
        ...
        for i, visit in enumerate(self.visits):
            ...
            if visit.date_of_recording.date() < visitdate.date():
